...
Class Php_Form_Builder{
	...
	private $config = array(/* Config array - can be overrided by passing in array in ini() */
        'default_input_type' => 'field_input'
    );
	 ...
	 * @return	string
	 */	
	function field_input($data = '', $value = '', $extra = '')
	{
		$defaults = array(
			'type' => 'text',
		...
		return '<input '.$this->_field_attributes($data, $defaults).$this->_field_attributes_to_string($extra)." />\n";
	 ...
	 * @return	string
	 */	
	function field_hidden($name, $value = '', $recursing = FALSE)
	{
		static $form;
		...
		if (is_array($name))
		{
			...
			{
				$this->field_hidden($key, $val, TRUE);
			}
	 ...
	 * @return	string
	 */	 
	function field_dropdown($data = '', $options = array(), $selected = array(), $extra = '')
	{
		$defaults = array();
		...
		$extra = $this->_field_attributes_to_string($extra);

		$multiple = (count($selected) > 1 && stripos($extra, 'multiple') === FALSE) ? ' multiple="multiple"' : '';

		$form = '<select '.rtrim($this->_field_attributes($data, $defaults)).$extra.$multiple.">\n";
	 ...
	 * @return	string
	 */
	function field_checkbox($data = '', $value = '', $checked = FALSE, $extra = '')
	{
	 ...
	 * @return	string
	 */
	function field_radio($data = '', $value = '', $checked = FALSE, $extra = '')
	{
	 ...
	 * @return	string
	 */	
	function field_textarea($data = '', $value = '', $extra = '')
	{
		$defaults = array(
			'name' => is_array($data) ? '' : $data,
			'cols' => '40',
		...
		return '<textarea '.$this->_field_attributes($data, $defaults).$this->_field_attributes_to_string($extra).'>'
	 ...
	 * @return	string
	 */
	function field_label($label_text = '', $id = '', $attributes = array())
	{
	 ...
	 * @return	string
	 */
	function field_submit($data = '', $value = '', $extra = '')
	{
		$defaults = array(
			'type' => 'submit',
			'name' => is_array($data) ? '' : $data,
	...
	function _form_open($action = '', $attributes = array(), $hidden = array())
	{
		...
		$attributes = $this->_field_attributes_to_string($attributes);
	...
	private function _label() {
        ...
        } elseif (isset($this->elm_options['id']) && $this->func != 'field_submit') {
            $label = $this->_make_field_label($this->elm_options['id']);
        }

        if ($this->func == 'field_submit') {
            $label = '';
        }

        return $this->field_label($label, $this->elm_options['name'], array(
            'class' => ''
        ));
    }
	...
	function build_form_field_input($options, $data_source = array()) {
        ...
        foreach ($options as $elm_options) {
            ...
            if (is_array($this->elm_options)) {
                ...
                $this->_field_prep_options();
                switch ($this->func) {
                    case 'field_hidden':
                        $this->print_string .= $this->_build_field_input();
                        break;
                    case 'field_checkbox':
                    case 'field_radio':
                        ...
                        foreach ($all_elm_options['options'] as $elm_suboptions) {
                            ...
                            $this->print_string .= $this->_build_field_input(FALSE);
	...
	private function _build_field_input($include_pre_post = true)
	{
        $input_html_string = '';
        
        if ($this->func == 'field_combine') {
            ...
            foreach ($elm_options_backup['elements'] as $elm) {
                ...
                $this->_field_prep_options(); /* Run Prep on the new one */
                if ($counter > 0 && !empty($elm_options_backup['combine_divider'])) {
                    $input_html_string .= $elm_options_backup['combine_divider'];
                }
                $input_html_string .= $this->_build_field_input(false);
                $counter++;
            }

            $this->elm_options = $elm_options_backup; /* We put our options back */
            $this->_field_prep_options(); /* Run Prep to restore the state in which we begain */
        } else {
            ...
            switch ($this->func) {
                ...
                case 'field_a':
                    ...
                    break;
                case 'field_label':
                    $input_html_string = $this->field_label($this->_make_field_label($this->elm_options['value']), '', array(
                        'class' => 'control-label text-left'
                    ));
                    break;
                case 'field_email':
                    $this->elm_options['type'] = 'email';
                    $input_html_string = $this->field_input($this->elm_options);
                    break;
                case 'field_tel':
                ...
                case 'field_submit':
                    ...
                    $input_html_string = $this->field_submit($name, $label, $this->_field_create_extra_string($this->elm_options));
                    break;
                case 'field_option':
                case 'field_dropdown':
                    ...
                    if (isset($this->elm_options['options']) && !empty($this->elm_options['options'])) {
                        ...
                        $input_html_string = $this->field_dropdown($name, $options, $value, $this->_field_create_extra_string());
                    } else {
                        // dump($this->elm_options);
                ...
                case 'field_textarea':
                    ...
                    $input_html_string = $this->field_textarea($this->elm_options);
                    break;
                case 'field_checkbox':
                    $input_html_string = $this->field_checkbox($this->elm_options);
                    break;
                case 'field_radio':
                    $input_html_string = $this->field_radio($this->elm_options);
                    break;
	...
	/* added for dropdown */
	private function _field_create_extra_string() {
        $extra = '';
	...
	private function _field_prep_options() {
        ...
        if (isset($this->elm_options['type']) && !empty($this->elm_options['type'])) {
            $this->func = 'field_' . $this->elm_options['type'];
	...
	function _field_attributes($attributes, $default)
	{
	...
	function _field_attributes_to_string($attributes)
	{
		if (empty($attributes))
		{
			return '';
